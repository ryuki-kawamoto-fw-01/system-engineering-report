...
from modules.text import romkan
from modules.model import WordEntry

# 現在の辞書パスを取得して修正
dictionary_path = ipadic.MECAB_ARGS.replace('"', "")
modified_dictionary_path = f"{dictionary_path} -Oyomi"
myomi = MeCab.Tagger(modified_dictionary_path)


# 辞書登録関数
def dictionary_registration(user_message: str, dictionary_list: List[WordEntry]):
    # 辞書のパスをログで確認
    logging.info(f"modified_dictionary_path: {modified_dictionary_path}")
    ...
    user_message_rev = inconsistency_collector_robust(
        tar_question=user_message,  # ユーザの質問文
        dictionary=dictionary_list,  # 辞書データを含むリスト
...
def inconsistency_collector_robust(
    ...
    dictionary: List[WordEntry] = [],  # 辞書データを含むリスト
    ...
    for value in dictionary:
        ...
        for tar_dict_txt in tar_dict_txt_list:
            ...
            # 辞書の対象語句と同じ長さのウィンドウで対象質問文を区切り、ローマ字統一表記において一致を検査
            partial_questions = []
            ...
            if verbose_detail:
                logging.info("partial_question      :" + most_similar_partial_question)
                logging.info("tar_dict_txt          :" + most_similar_tar_dict_txt)
                logging.info(
                    "conv_partial_question :" + most_similar_conv_partial_question
                )
                logging.info("conv_tar_dict_txt     :" + most_similar_conv_tar_dict_txt)
                logging.info("dist                  :" + str(most_similar_dist) + "\n")
            ...
            if verbose and len(similar_texts) > 0:
                record = {
                    "検出語句": similar_texts,
                    f"辞書語句({value.terms})": tar_dict_txt,
                    f"統一語句({value.uniform_name})": tar_univorm_name_dict_txt,
                }
