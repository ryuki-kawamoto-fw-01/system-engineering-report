...
EXC_TO_TABLE: dict[Type[Exception], tuple[str, ErrorCode]] = {
    RateLimitError: ("E_B1_00030", ErrorCode.RATE_LIMIT),
    APIConnectionError: ("E_B1_00040", ErrorCode.SERVICE_UNAVAILABLE),
    APITimeoutError: ("E_B1_00040", ErrorCode.SERVICE_UNAVAILABLE),
    APIError: ("E_B1_00040", ErrorCode.SERVICE_UNAVAILABLE),
    AuthenticationError: (
        "E_B1_00050",
    ...
    InternalServerError: ("E_B1_00010", ErrorCode.INTERNAL_ERROR),
    NotFoundError: (
        "E_B1_00010",
        ErrorCode.NOT_FOUND,
    ),  # 現状エラー定義のエクセル表から無理やり持ってきているが、必要に応じてより特化したエラーメッセージを追加する
    ...
    ValueError: ("E_B1_00060", ErrorCode.BAD_REQUEST),
    KeyError: ("E_B1_00070", ErrorCode.BAD_REQUEST),
    LLMSpecificError: ("", ErrorCode.INTERNAL_ERROR),  # LLMSpecificError は動的に決まる
    Exception: ("E_B1_00080", ErrorCode.INTERNAL_ERROR),
}
    ...
    def _resolve_error_table(
        ...
        if isinstance(err, BadRequestError):
            ...
            if _TOKEN_OVERRUN_RE.search(raw_msg):
                return "E_B1_00020", ErrorCode.BAD_REQUEST, {"value": "メッセージ"}
            ...
            if isinstance(details, dict):
                ...
                if cfr:
                    ...
                    return "E_B1_00100", ErrorCode.BAD_REQUEST, {}
            # 3) それ以外
            return "E_B1_00110", ErrorCode.BAD_REQUEST, {}
