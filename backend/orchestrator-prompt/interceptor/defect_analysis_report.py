...
class DefectAnalysisReportInterceptor(ErrorHandler):
    @classmethod
...
def defect_analysis_report_error_handler(func: Callable) -> Callable:
    def wrapper(req: azure_func.HttpRequest) -> azure_func.HttpResponse:
        try:
            return func(req)
        except Exception as e:
            logging.error(f"Exception occurred: {str(e)}")
            error_response, status_code = DefectAnalysisReportInterceptor.handle_error(
                e, req
            )
