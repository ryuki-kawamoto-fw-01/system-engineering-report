...
class SupposedQuestionInterceptor(ErrorHandler):
    @classmethod
    def _resolve_error_table(
        cls, err: Exception, req: azure_func.HttpRequest
    ) -> tuple[str, ErrorCode, dict[str, Any]]:
        # 想定質問はJSONボディでBlobパスを受け取るため、req.filesは空
        # 暗号化ファイルのエラーはget_file_content内でLLMSpecificErrorとして発生する
        # LLMSpecificErrorの場合は親クラスで適切に処理される
        if req.files:
            # ファイルの暗号化チェック（念のため）
...
def supposed_question_error_handler(
    ...
    def supposed_question_wrapper(
        req: azure_func.HttpRequest,
    ) -> azure_func.HttpResponse:
        try:
            return func(req)
        except Exception as e:  # noqa: BLE001
            error_response, status_code = SupposedQuestionInterceptor.handle_error(
                e, req
            )
    ...
    supposed_question_wrapper.__name__ = func.__name__  # デプロイ時の関数名を維持
    return supposed_question_wrapper
