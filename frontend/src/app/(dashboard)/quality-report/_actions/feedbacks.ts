...
export default async function feedbackQualityReport(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await qualityReportContainer.item(id, user.id).read<QualityReportModel>();
    ...
    if (!resource) {
      throw new Error(`Quality report with id ${id} not found`);
    }

    await qualityReportContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
    ...
    console.error('Feedback quality report error:', error);
    return {
      success: false,
