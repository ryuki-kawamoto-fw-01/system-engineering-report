...
export default async function feedbackMarketResearchReport(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await marketresearchContainer
      .item(id, user.id)
      .read<MarkerResearchReportModel>();
    ...
    await marketresearchContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
