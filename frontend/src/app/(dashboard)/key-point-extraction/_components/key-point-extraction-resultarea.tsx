...
import { setFeedbackAt, setResult } from '@/app/_store/slice/key-point-extraction';
...
export default function KeyPointExtractionResultArea({
  keyPointExtractionResult,
  className,
}: Props): JSX.Element {
  const dispatch = useAppDispatch();
  const { id, feedbackAt } = useAppSelector((state) => state.keyPointExtraction);

  const [preEditContent, setPreEditContent] = useState(keyPointExtractionResult);
  const [isEditing, setIsEditing] = useState(false);

  useEffect(() => {
    setPreEditContent(keyPointExtractionResult);
  }, [keyPointExtractionResult]);

  const copyResult = (event: React.MouseEvent<HTMLButtonElement>) => {
    event.preventDefault();
    navigator.clipboard.writeText(preEditContent);
    toast.success(getMessage('I_F_00050', '要点抽出結果'));
  };
  ...
  function downloadMessage() {
    ...
    element.download = `要点抽出結果_${timestamp}.txt`;
  ...
  const handleSave = () => {
    dispatch(setResult({ keyPointExtractionResult: preEditContent, feedbackAt }));
    setIsEditing(false);
  };
  ...
  return (
    <div className={className ?? 'flex flex-col'}>
      <div className="mb-1 flex min-h-8 items-end justify-between">
        <Label className="text-base">抽出結果</Label>
        {!isEditing ? (
          <div className="flex items-center">
            <FeedbackGoodButton
              source="keyPointExtraction"
              messageId={id}
      ...
      <div className="relative h-full">
        ...
        <Textarea
          ...
          placeholder="ここに要点抽出した結果が表示されます"
          readOnly={!isEditing}
          className="h-full"
        />
      </div>
