...
export default async function feedbackKeyPointExtraction(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await keyPointExtractionContainer
      .item(id, user.id)
      .read<KeyPointExtractionModel>();
    ...
    if (!resource) {
      throw new Error(`keyPointExtraction with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`keyPointExtraction with id ${id} is already fed back`);
    }

    await keyPointExtractionContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
