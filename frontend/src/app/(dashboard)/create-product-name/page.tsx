...
import CreateProductNameTitle from './_components/create-product-name-title';
import ProductNameFormArea from './_components/product-name-form-area';
import ProductNameResults from './_components/product-name-results';

export default function Layout() {
  const { result } = useAppSelector((state) => state.createProductName);
  const { layout, isTwoColumns, isLeftOnly, isRightOnly, switchLayout } = useUseCaseLayout(result);

  return (
    <PageLayout className="flex flex-col">
      <CreateProductNameTitle />
      ...
      <div className="flex min-h-0 flex-1 gap-x-10 overflow-hidden">
        ...
        {(isRightOnly || isTwoColumns) && (
          <ProductNameResults className={cn('w-full', isTwoColumns && 'w-2/3 min-h-0')} />
        )}
      </div>
    </PageLayout>
  );
