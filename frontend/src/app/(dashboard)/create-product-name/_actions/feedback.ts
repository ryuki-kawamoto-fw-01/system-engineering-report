...
import { ProductNameModel } from '../../../../../config';
import { createProductNameContainer } from '../../../../../cosmos';

type Response = Result;

export default async function feedbackProductName(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await createProductNameContainer
      .item(id, user.id)
      .read<ProductNameModel>();
    ...
    if (!resource) {
      throw new Error(`Product Name with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`Product Name with id ${id} is already fed back`);
    }

    await createProductNameContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
