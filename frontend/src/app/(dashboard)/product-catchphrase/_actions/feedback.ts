...
export default async function feedbackCatchphrase(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await productCatchphraseContainer
      .item(id, user.id)
      .read<CatchphraseModel>();
    ...
    if (!resource) {
      throw new Error(`Catchphrase with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`Catchphrase with id ${id} is already fed back`);
    }

    await productCatchphraseContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
