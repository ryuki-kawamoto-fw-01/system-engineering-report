...
import { setResult, setFeedbackAt } from '@/app/_store/slice/product-catchphrase';
...
export default function CatchphraseResultArea({ className }: Props) {
  const dispatch = useAppDispatch();
  const { result, id, feedbackAt } = useAppSelector((state) => state.productCatchphrase);
  ...
  function downloadMessage() {
    ...
    element.download = `キャッチコピー_${timestamp}.txt`;
  ...
  return (
    <div className={className}>
      ...
      <div className="relative h-full">
        ...
        {!isEditing ? (
          <ScrollArea
            className="size-full rounded-lg border bg-white px-4 py-2 shadow"
            ref={markdownRef}
          >
            {preEditContent ? (
              <Markdown>{preEditContent.replace(/<\/?b>/g, '**')}</Markdown>
            ) : (
              <span className="text-base text-gray-400">
                ここに生成されたキャッチコピーが表示されます
              </span>
            )}
          </ScrollArea>
        ) : (
          <Textarea
            ...
            placeholder="ここに生成されたキャッチコピーが表示されます"
            readOnly={!isEditing}
            className="h-full"
          />
        )}
