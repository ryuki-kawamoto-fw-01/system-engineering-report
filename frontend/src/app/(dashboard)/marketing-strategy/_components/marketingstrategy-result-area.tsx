...
import { setResult, setFeedbackAt } from '@/app/_store/slice/marketingstrategy';
...
export default function ResultArea({ className }: Props) {
  ...
  const { result, id, feedbackAt } = useAppSelector((state) => state.marketingstrategy);
  ...
  function downloadMessage() {
    ...
    element.download = `マーケティング戦略_${timestamp}.txt`;
  ...
  return (
    <div className={className}>
      <div className="mb-1 flex min-h-8 items-end justify-between">
        <Label className="text-base">マーケティング資料作成結果</Label>
        {!isEditing ? (
      ...
      <div className="relative h-full">
        ...
        <Textarea
          ...
          placeholder="ここに生成されたマーケティング戦略資料が表示されます"
          readOnly={!isEditing}
          className="h-full"
        />
      </div>
