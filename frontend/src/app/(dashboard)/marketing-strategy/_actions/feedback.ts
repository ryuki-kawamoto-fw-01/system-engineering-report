...
export default async function feedbackMarketingStrategy(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await marketingstrategyContainer
      .item(id, user.id)
      .read<MarketingStrategyModel>();
    ...
    await marketingstrategyContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
