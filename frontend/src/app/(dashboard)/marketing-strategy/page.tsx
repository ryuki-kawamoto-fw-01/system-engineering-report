...
import FormArea from './_components/marketingstrategy-form';
import Results from './_components/marketingstrategy-result';
import MarketingStrategyTitle from './_components/marketingstrategy-title';

export default function Layout() {
  const { result } = useAppSelector((state) => state.marketingstrategy);
  const { layout, isTwoColumns, isLeftOnly, isRightOnly, switchLayout } = useUseCaseLayout(result);

  return (
    <PageLayout className="flex flex-col">
      <MarketingStrategyTitle />
