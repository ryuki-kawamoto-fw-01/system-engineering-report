...
export default async function feedbackTechassess(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await techassessContainer.item(id, user.id).read<TechassessModel>();
    ...
    if (!resource) {
      throw new Error(`Techassess with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`Techassess with id ${id} is already fed back`);
    }

    await techassessContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
