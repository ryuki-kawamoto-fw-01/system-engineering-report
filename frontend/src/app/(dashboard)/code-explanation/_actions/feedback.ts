...
export default async function feedbackCodeExplanation(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await codeExplanationContainer
      .item(id, user.id)
      .read<CodeExplanationModel>();
    ...
    if (!resource) {
      throw new Error(`code explanation with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`code explanation with id ${id} is already fed back`);
    }

    await codeExplanationContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
