...
export default async function feedbackTechnologyProposal(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await createTechnologyProposalContainer
      .item(id, user.id)
      .read<TechnologyProposalModel>();
    ...
    if (!resource) {
      throw new Error(`TechnologyProposal with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`TechnologyProposal with id ${id} is already fed back`);
    }

    await createTechnologyProposalContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
