...
export default async function feedbackAdviceReact(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await adviceReactContainer.item(id, user.id).read<AdviceReactModel>();
    ...
    await adviceReactContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
