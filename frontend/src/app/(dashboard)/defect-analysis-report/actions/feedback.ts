...
export default async function feedbackDefectAnalysisReport(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await defectAnalysisReportContainer
      .item(id, user.id)
      .read<DefectAnalysisReportModel>();
    ...
    if (!resource) {
      throw new Error(`Defect analysis report with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`Defect analysis report with id ${id} is already fed back`);
    }

    await defectAnalysisReportContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
