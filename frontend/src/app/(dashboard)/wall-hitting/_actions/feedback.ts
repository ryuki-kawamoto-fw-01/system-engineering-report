...
export default async function feedbackWallHitting(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await wallHittingContainer.item(id, user.id).read<WallHittingModel>();
    ...
    if (!resource) {
      throw new Error(`WallHitting with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`WallHitting with id ${id} is already fed back`);
    }

    await wallHittingContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
