...
import { agentMessageContainer } from '../../../../../../../cosmos';

type Response = Result;

export default async function feedbackAgentChatMessage(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource: message } = await agentMessageContainer
      .item(id, user.id)
    ...
    await agentMessageContainer.items.upsert({
      ...message,
      feedbackType: ftype,
    ...
    revalidatePath('/agent/specification');
