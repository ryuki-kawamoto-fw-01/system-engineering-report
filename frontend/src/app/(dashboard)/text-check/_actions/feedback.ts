...
export default async function feedbackTextCheck(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await textCheckContainer.item(id, user.id).read<TextCheckModel>();
    ...
    if (!resource) {
      throw new Error(`TextCheck with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`TextCheck with id ${id} is already fed back`);
    }

    await textCheckContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
