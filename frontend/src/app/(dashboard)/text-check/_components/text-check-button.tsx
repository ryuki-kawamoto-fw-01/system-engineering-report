...
export default function TextCheckButton({ selectedTab }: Props) {
  const {
    watch,
    formState: { isSubmitting },
  } = useFormContext();
  const { text, fileList, content1 } = watch();

  const isDisabled =
    selectedTab === 'file-upload'
      ? !fileList || fileList.length === 0 || !content1 || content1.trim() === '' || isSubmitting
      : !text || text.trim() === '' || !content1 || content1.trim() === '' || isSubmitting;
