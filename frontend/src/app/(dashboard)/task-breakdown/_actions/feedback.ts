...
export default async function feedbackTaskBreakdown(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await taskBreakdownContainer.item(id, user.id).read<TaskBreakdownModel>();
    ...
    if (!resource) {
      throw new Error(`taskBreakdown with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`taskBreakdown with id ${id} is already fed back`);
    }

    await taskBreakdownContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
