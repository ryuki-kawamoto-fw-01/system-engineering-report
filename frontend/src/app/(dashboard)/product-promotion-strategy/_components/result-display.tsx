...
import { setResult, setFeedbackAt } from '@/app/_store/slice/product-promotion-strategy';
import type { RootState } from '@/app/_store/store';
import { getMessage } from '@/app/_utils/message';
import { cn } from '@/app/_utils/tw-merge';

interface ProductPromotionStrategyResultProps {
  className?: string;
}

export default function ProductPromotionStrategyResult({
  className,
}: ProductPromotionStrategyResultProps) {
  const dispatch = useDispatch();
  const { result, id, feedbackAt } = useSelector(
    (state: RootState) => state.productPromotionStrategy
  );
  ...
  const copyResult = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    toast.success(getMessage('I_F_00050', '拡販戦略'));
  };
  ...
  const downloadResult = () => {
    ...
    element.download = `拡販戦略_${timestamp}.txt`;
    document.body.appendChild(element);
    element.click();
    document.body.removeChild(element);
    toast.success(getMessage('I_F_00040', '拡販戦略'));
  };

  return (
    <div className={cn('flex h-full flex-col', className)}>
      <div className="mb-1 flex items-center justify-between">
        <Label className="text-base">拡販戦略</Label>
        {!isEditing ? (
          <div className="flex items-center">
            <FeedbackGoodButton
              source="productPromotionStrategy"
              messageId={id}
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button type="button" variant="icon" size="icon" onClick={downloadResult}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button type="button" size="icon" variant="icon" onClick={handleEdit}>
        ...
        {isEditing ? (
          ...
          <ScrollArea className="h-full rounded-lg border bg-white px-4 py-2 shadow">
            {result ? (
              ...
              <span className="text-base text-gray-400">ここに拡販戦略が表示されます</span>
            )}
          </ScrollArea>
        )}
      </div>
