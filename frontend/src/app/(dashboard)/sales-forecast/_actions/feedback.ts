...
export default async function feedbackSalesForecast(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await salesForecastContainer.item(id, user.id).read<SalesForecastModel>();
    ...
    if (!resource) {
      throw new Error(`salesForecast with id ${id} not found`); // データがなければエラー
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`salesForecast with id ${id} is already feed back`); // 既にフィードバック済みならエラー
    }

    await salesForecastContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
