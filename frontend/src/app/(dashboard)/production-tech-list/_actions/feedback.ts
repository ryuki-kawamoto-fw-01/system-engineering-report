...
export default async function feedbackProductionTechList(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await productionTechListContainer
      .item(id, user.id)
      .read<ProductionTechListModel>();
    ...
    if (resource.feedbackType !== undefined) {
      throw new Error(`summary with id ${id} is already fed back`);
    }

    await productionTechListContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
