...
export default async function feedbackDesignDocumentReview(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await designDocumentReviewContainer
      .item(id, user.id)
      .read<DesignDocumentReviewModel>();
    ...
    if (!resource) {
      throw new Error(`createMinutes with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`createMinutes with id ${id} is already fed back`);
    }

    await designDocumentReviewContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
