...
export default async function feedbackNeedsSurvey(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await needsSurveyContainer.item(id, user.id).read<NeedsSurveyModel>();
    ...
    await needsSurveyContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
