...
export default async function feedbackCorporateSurvey(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await corporateSurveyContainer
      .item(id, user.id)
      .read<CorporateSurveyModel>();
    ...
    if (!resource) {
      throw new Error(`corporateSurvery with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`corporateSurvery with id ${id} is already fed back`);
    }

    await corporateSurveyContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
