...
export default async function feedbackSummary(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await summaryContainer.item(id, user.id).read<SummaryModel>();
    ...
    await summaryContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
