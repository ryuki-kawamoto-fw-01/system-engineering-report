...
export default async function feedbackJudge(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await judgeIdeaContainer.item(id, user.id).read<JudgeModel>();
    ...
    await judgeIdeaContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
