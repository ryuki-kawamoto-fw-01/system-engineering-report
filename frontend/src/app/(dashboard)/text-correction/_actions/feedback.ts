...
export default async function feedbackTextCorrection(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await textCorrectionContainer
      .item(id, user.id)
      .read<TextCorrectionModel>();
    ...
    if (!resource) {
      throw new Error(`textCorrection with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`textCorrection with id ${id} is already fed back`);
    }

    await textCorrectionContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
