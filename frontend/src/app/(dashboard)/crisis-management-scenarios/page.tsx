...
import CrisisManagementScenariosTitle from './_components/crisis-management-scenarios-title';
import FormArea from './_components/form-area';
import Results from './_components/results';

export default function Layout() {
  const { result } = useAppSelector((state) => state.crisisManagementScenarios);
  const { layout, isTwoColumns, isLeftOnly, isRightOnly, switchLayout } = useUseCaseLayout(result);

  return (
    <PageLayout className="flex flex-col">
      <CrisisManagementScenariosTitle />
