...
export default async function feedbackCrisisManagementScenarios(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await crisisManagementScenariosContainer
      .item(id, user.id)
    ...
    await crisisManagementScenariosContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
