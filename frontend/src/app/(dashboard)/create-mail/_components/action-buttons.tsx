...
import { setFeedbackAt } from '@/app/_store/slice/create-mail';
...
type Props = {
  ...
  handleCancel: () => void;
  handleSave: () => void;
  result?: string; // Optional, if needed for download functionality
};
...
export default function ActionButtons({
  ...
  const { createdSubject, createdContent, id, feedbackAt } = useAppSelector(
  ...
  function downloadMessage() {
    ...
    const file = new Blob([`件名\n${createdSubject}\n\n本文\n${createdContent}`], {
      type: 'text/plain',
    });
    element.href = URL.createObjectURL(file);
    element.download = `メール作成_${timestamp}.txt`;
