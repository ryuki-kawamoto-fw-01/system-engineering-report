...
import { NewMailModel } from '../../../../../config';
import { createNewMailContainer } from '../../../../../cosmos';

type Response = Result;

export default async function feedbackMail(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await createNewMailContainer.item(id, user.id).read<NewMailModel>();
    ...
    if (!resource) {
      throw new Error(`createMail with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`createMail with id ${id} is already fed back`);
    }

    await createNewMailContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
