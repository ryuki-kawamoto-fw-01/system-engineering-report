...
export default async function feedbackQualityStandardDocument(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await qualityStandardDocumentContainer
      .item(id, user.id)
    ...
    if (!resource) {
      throw new Error(`Quality Standard Document with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`Quality Standard Document with id ${id} is already fed back`);
    }

    await qualityStandardDocumentContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
