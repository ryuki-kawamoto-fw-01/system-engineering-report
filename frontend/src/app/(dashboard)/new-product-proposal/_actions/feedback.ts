...
export default async function feedbackNewProductProposal(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await newProductProposalContainer
      .item(id, user.id)
      .read<NewProductProposalModel>();
    ...
    await newProductProposalContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
