...
export default async function feedbackTroubleShootingGuide(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await troubleShootingGuideContainer
      .item(id, user.id)
      .read<TroubleShootingGuideModel>();
    ...
    if (!resource) {
      throw new Error(`trouble shooting guide with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`trouble shooting guide with id ${id} is already fed back`);
    }

    await troubleShootingGuideContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
