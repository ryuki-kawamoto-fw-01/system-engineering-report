...
export default async function feedbackSupposedQuestion(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await supposedQuestionContainer
      .item(id, user.id)
      .read<SupposedQuestionModel>();
    ...
    if (!resource) {
      throw new Error(`supposedQuestion with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`supposedQuestion with id ${id} is already fed back`);
    }

    await supposedQuestionContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
