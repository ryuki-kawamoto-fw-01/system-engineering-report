...
export default async function feedbackErrorAnalysis(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await errorAnalysisContainer.item(id, user.id).read<ErrorAnalysisModel>();
    const ftype = feedbackType;

    // feedbackOptionsに数値が含まれている場合、対応する変数に1を代入
    let foption1 = 0;
    let foption2 = 0;
    let foption3 = 0;
    let foption4 = 0;
    ...
    if (!resource) {
      throw new Error(`Error analysis with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`Error analysis with id ${id} is already fed back`);
    }

    await errorAnalysisContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
