...
export default async function feedbackCreateDesignDocument(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await createDesignDocumentContainer
      .item(id, user.id)
      .read<CreateDesignDocumentModel>();
    ...
    if (resource.feedbackType !== undefined) {
      throw new Error(`Idea with id ${id} is already fed back`);
    }

    await createDesignDocumentContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
