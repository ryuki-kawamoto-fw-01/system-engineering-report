...
export default async function feedbackBrainstorming(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await brainstormingContainer.item(id, user.id).read<BrainstormingModel>();
    ...
    await brainstormingContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
