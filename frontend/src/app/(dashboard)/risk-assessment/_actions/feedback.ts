...
export default async function feedbackRiskAssessment(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await riskAssessmentContainer
      .item(id, user.id)
      .read<RiskAssessmentModel>();
    ...
    await riskAssessmentContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
