...
export default async function feedbackCompanyAnalysis(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await companyAnalysisContainer
      .item(id, user.id)
      .read<CompanyAnalysisModel>();
    ...
    if (!resource) {
      throw new Error(`companyAnalysis with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`companyAnalysis with id ${id} is already fed back`);
    }

    await companyAnalysisContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
