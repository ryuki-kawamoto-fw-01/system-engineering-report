...
export default async function feedbackIdea(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await createIdeaContainer.item(id, user.id).read<IdeaModel>();
    ...
    await createIdeaContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
