...
export default async function feedbackBusinessPlan(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await businessPlanContainer.item(id, user.id).read<BusinessPlanModel>();
    ...
    await businessPlanContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
