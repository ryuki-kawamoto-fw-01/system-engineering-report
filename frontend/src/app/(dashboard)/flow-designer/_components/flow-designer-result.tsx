...
import { setResult, setFeedbackAt } from '@/app/_store/slice/flow-designer';
...
export default function FlowDesignerResult({ className }: Props) {
  const dispatch = useAppDispatch();
  const { result, id, feedbackAt } = useAppSelector((state) => state.flowDesigner);
  ...
  useEffect(() => {
    ...
    if (result) {
      console.log(
        'FlowDesignerResult - result received and displaying:',
        result.length,
        'characters'
      );
    }
  ...
  const copyResult = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    toast.success(getMessage('I_F_00050', '工程管理表'));
  };
  ...
  const downloadResult = () => {
    ...
    element.download = `工程管理表_${timestamp}.txt`;
  ...
  return (
    <div className={cn('flex h-full flex-col', className)}>
      <div className="mb-1 flex items-center justify-between">
        <Label className="text-base">工程管理表</Label>
        {!isEditing ? (
      ...
      <div className="relative h-full">
        ...
        {isEditing ? (
          <Textarea
            ...
            placeholder="ここに生成した工程管理表が表示されます"
            readOnly={false}
            className="h-full"
          />
        ) : (
