...
export default async function feedbackFlowDesigner(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await flowDesignerContainer.item(id, user.id).read<FlowDesignerModel>();
    ...
    if (!resource) {
      throw new Error(`FlowDesigner with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`FlowDesigner with id ${id} is already fed back`);
    }

    await flowDesignerContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
