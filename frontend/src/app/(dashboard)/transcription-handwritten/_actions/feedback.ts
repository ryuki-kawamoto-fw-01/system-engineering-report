...
export default async function feedbackTranscriptionHandwritten(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await transcriptionHandwrittenContainer
      .item(id, user.id)
    ...
    if (!resource) {
      throw new Error(`transcriptionHandwritten with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`transcriptionHandwritten with id ${id} is already fed back`);
    }

    await transcriptionHandwrittenContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
