...
import { getSasUrl, uploadWithSasUrl } from '@/app/(dashboard)/create-manual/_actions/uploadFile';
import SvgClose from '@/app/_components/icon/button/Close';
import SvgTextFile from '@/app/_components/icon/button/TextFile';
...
import { formatFileSize, splitFileName } from '@/app/_utils/file';
...
export default function FileDropArea({
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      // 動画ファイルは1つまでに制限
      if (urls.length > 0) {
        toast.error('動画ファイルは1つまでアップロード可能です');
        return;
      }
      ...
      try {
        ...
        // ステップ1: SAS URLを取得
        const sasResult = await getSasUrl(file.name);

        if (!sasResult.success || !sasResult.uploadUrl) {
          throw new Error('SAS URL取得失敗');
        }

        // ステップ2: サーバーアクション経由でBlobにアップロード
        const formData = new FormData();
        formData.append('file', file);
        formData.append('uploadUrl', sasResult.uploadUrl);
        const uploadResult = await uploadWithSasUrl(formData);
        ...
        onChangeField({ [name]: [...currentUrls, sasResult.blobUrl] });
    ...
    [name, onChangeField, urls, setIsUpload]
  );
  ...
  const onChange = useCallback(
    ...
    [fileRef, name, onChangeField, maxSize, minSize, accept, urls]
  );
            ...
            <FormControl>
              ...
              >
                ...
                <div className="mt-1.5 text-left text-2xs font-normal">
                  ...
                  {/* 動画の長さはContent Understandingの処理上限30分で設定 */}
                  <div>動画の長さ：30分以内</div>
                  <div>解像度：320×240px〜1920×1080px</div>
                  <div>処理時間の目安：2分程度</div>
                </div>
                {isUpload && (
