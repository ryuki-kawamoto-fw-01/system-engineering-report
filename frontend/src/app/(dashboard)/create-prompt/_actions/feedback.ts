...
export default async function feedbackCreatePrompt(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await promptContainer.item(id, user.id).read<CreatePromptModel>();
    ...
    if (resource.feedbackType !== undefined) {
      throw new Error(`prompt with id ${id} is already fed back`);
    }

    await promptContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
