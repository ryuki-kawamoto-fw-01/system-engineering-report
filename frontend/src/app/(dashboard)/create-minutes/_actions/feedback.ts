...
export default async function feedbackCreateMinutes(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await createMinutesContainer
      .item(id, user.id)
      .read<CreateCreateMinutesModel>();
    ...
    await createMinutesContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
