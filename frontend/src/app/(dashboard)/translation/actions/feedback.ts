...
export default async function feedbackTranslation(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await translateContainer.item(id, user.id).read<TranslationModel>();
    ...
    if (resource.feedbackType !== undefined) {
      throw new Error(`translation with id ${id} is already fed back`);
    }

    await translateContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
