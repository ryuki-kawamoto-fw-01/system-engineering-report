...
import { setFeedbackAt, setResult } from '@/app/_store/slice/talk-script';
...
export default function TalkScriptResultArea({ className }: Props) {
  const dispatch = useAppDispatch();
  const { result, id, feedbackAt } = useAppSelector((state) => state.talkScript);
  ...
  function downloadMessage() {
    ...
    element.download = `トークスクリプト_${timestamp}.txt`;
    ...
    <div className={className}>
      ...
      <div className="relative h-full">
        ...
        <Textarea
          ...
          placeholder="ここに生成されたトークスクリプトが表示されます"
          readOnly={!isEditing}
          className="h-full"
        />
      </div>
