...
export default async function feedbackTalkScript(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await talkScriptContainer.item(id, user.id).read<TalkScriptModel>();
    ...
    if (!resource) {
      throw new Error(`talkScript with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`talkScript with id ${id} is already fed back`);
    }

    await talkScriptContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
