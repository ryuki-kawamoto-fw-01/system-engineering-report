...
export default async function feedbackProductAARRR(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await productAARRRContainer.item(id, user.id).read<ProductAARRRModel>();
    ...
    if (!resource) {
      throw new Error(`Product AARRR with id ${id} not found`);
    }

    if (resource.feedbackType !== undefined) {
      throw new Error(`Product AARRR with id ${id} is already fed back`);
    }

    await productAARRRContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
