...
import SvgBad from '@/app/_components/icon/button/Bad';
import SvgDisabledBad from '@/app/_components/icon/button/DisabledBad';
...
import { badFeedbackOptions } from '../../../../config';
...
export default function FeedbackBadButton({
  source,
  messageId,
  isSubmitted,
  handleSubmit,
  ...
  const onSubmit = async (data: z.infer<typeof FeedbackSchema>) => {
    ...
    if (source === 'createPrompt') {
      res = await feedbackCreatePrompt(messageId, 0, data.reasons, data.text);
    } else if (source === 'idea') {
      res = await feedbackIdea(messageId, 0, data.reasons, data.text);
    } else if (source === 'corporateSurvey') {
      res = await feedbackCorporateSurvey(messageId, 0, data.reasons, data.text);
    } else if (source === 'minutes') {
      res = await feedbackCreateMinutes(messageId, 0, data.reasons, data.text);
    } else if (source === 'companyAnalysis') {
      res = await feedbackCompanyAnalysis(messageId, 0, data.reasons, data.text);
    } else if (source === 'defectAnalysisReport') {
      res = await feedbackDefectAnalysisReport(messageId, 0, data.reasons, data.text);
    } else if (source === 'supposedQuestion') {
      res = await feedbackSupposedQuestion(messageId, 0, data.reasons, data.text);
    } else if (source === 'talkScript') {
      res = await feedbackTalkScript(messageId, 0, data.reasons, data.text);
    } else if (source === 'technologyTraining') {
      res = await feedbackTechnologyTraining(messageId, 0, data.reasons, data.text);
    } else if (source === 'textCorrection') {
      res = await feedbackTextCorrection(messageId, 0, data.reasons, data.text);
    } else if (source === 'translation') {
      res = await feedbackTranslation(messageId, 0, data.reasons, data.text);
    } else if (source === 'mail') {
      res = await feedbackMail(messageId, 0, data.reasons, data.text);
    } else if (source === 'catchphrase') {
      res = await feedbackCatchphrase(messageId, 0, data.reasons, data.text);
    } else if (source === 'summary') {
      res = await feedbackSummary(messageId, 0, data.reasons, data.text);
    } else if (source === 'needsSurvey') {
      res = await feedbackNeedsSurvey(messageId, 0, data.reasons, data.text);
      ...
      res = await feedbackProductServiceBenefitIdea(messageId, 0, data.reasons, data.text);
    } else if (source === 'codeExplanation') {
      res = await feedbackCodeExplanation(messageId, 0, data.reasons, data.text);
    } else if (source === 'marketresearchReport') {
      res = await feedbackMarketResearchReport(messageId, 0, data.reasons, data.text);
    } else if (source === 'brainstorming') {
      res = await feedbackBrainstorming(messageId, 0, data.reasons, data.text);
    } else if (source === 'productionTechList') {
      res = await feedbackProductionTechList(messageId, 0, data.reasons, data.text);
    } else if (source === 'crisisManagementScenarios') {
      res = await feedbackCrisisManagementScenarios(messageId, 0, data.reasons, data.text);
    } else if (source === 'error-analysis') {
      res = await feedbackErrorAnalysis(messageId, 0, data.reasons, data.text);
    } else if (source === 'qualityReport') {
      res = await feedbackQualityReport(messageId, 0, data.reasons, data.text);
    } else if (source === 'keyPointExtraction') {
      res = await feedbackKeyPointExtraction(messageId, 0, data.reasons, data.text);
    } else if (source === 'risk-assessment') {
      res = await feedbackRiskAssessment(messageId, 0, data.reasons, data.text);
    } else if (source === 'judge') {
      res = await feedbackJudge(messageId, 0, data.reasons, data.text);
    } else if (source === 'transcriptionHandwritten') {
      res = await feedbackTranscriptionHandwritten(messageId, 0, data.reasons, data.text);
    } else if (source === 'troubleShooting') {
      res = await feedbackTroubleShootingGuide(messageId, 0, data.reasons, data.text);
    } else if (source === 'taskBreakdown') {
      res = await feedbackTaskBreakdown(messageId, 0, data.reasons, data.text);
    } else if (source === 'schedule') {
      res = await feedbackSchedule(messageId, 0, data.reasons, data.text);
    } else if (source === 'incidentReport') {
      res = await feedbackIncidentReport(messageId, 0, data.reasons, data.text);
    } else if (source === 'textCheck') {
      res = await feedbackTextCheck(messageId, 0, data.reasons, data.text);
    } else if (source === 'salesForecast') {
      res = await feedbackSalesForecast(messageId, 0, data.reasons, data.text);
    } else if (source === 'businessPlan') {
      res = await feedbackBusinessPlan(messageId, 0, data.reasons, data.text);
    } else if (source === 'researchReport') {
      res = await feedbackResearchReport(messageId, 0, data.reasons, data.text);
    } else if (source === 'product-aarrr') {
      res = await feedbackProductAARRR(messageId, 0, data.reasons, data.text);
    } else if (source === 'qualityStandardDocument') {
      res = await feedbackQualityStandardDocument(messageId, 0, data.reasons, data.text);
    } else if (source === 'marketingstrategy') {
      res = await feedbackMarketingStrategy(messageId, 0, data.reasons, data.text);
      ...
      res = await feedbackCreateDesignDocument(messageId, 0, data.reasons, data.text);
    } else if (source === 'productPromotionStrategy') {
      res = await feedbackProductPromotionStrategy(messageId, 0, data.reasons, data.text);
    } else if (source === 'flowDesigner') {
      res = await feedbackFlowDesigner(messageId, 0, data.reasons, data.text);
    } else if (source === 'techassess') {
      res = await feedbackTechassess(messageId, 0, data.reasons, data.text);
    } else if (source === 'image') {
      res = await feedbackImage(messageId, 0, data.reasons, data.text);
    } else if (source === 'design-document-review') {
      res = await feedbackDesignDocumentReview(messageId, 0, data.reasons, data.text);
    }
    ...
    <div>
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              {isSubmitted ? <SvgDisabledBad className="size-5" /> : <SvgBad className="size-5" />}
            </Button>
          </TooltipTrigger>
          <TooltipContent>
            <p>この回答は改善が必要です</p>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent>
          ...
          <div>
            <p>より良いサービスをご提供するにあたり、追加のフィードバックをお願いいたします。</p>
            <Form {...form}>
              <form className="mt-6">
                <FormField
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="mt-6">
                      <OptionalLabel>
                        特に気になった点・もっとこうしてほしい点があれば教えてください。
                      </OptionalLabel>
                      <FormControl>
                        <Input
                          placeholder="問題を改善し、より良いサービスをご提供するために、どのような回答を期待していたのか教えてください。"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
