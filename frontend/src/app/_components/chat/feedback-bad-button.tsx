...
export default function FeedbackBadButton({
  ...
  const onSubmit = async (data: z.infer<typeof FeedbackSchema>) => {
    ...
    if (source === 'rag') {
      res = await feedbackRagChatMessage(messageId, 0, data.reasons, data.text);
    } else if (source === 'chat') {
      res = await feedbackChatMessage(messageId, 0, data.reasons, data.text);
    } else if (source === 'agent') {
      res = await feedbackAgentChatMessage(messageId, 0, data.reasons, data.text);
      // } else if (source === 'voice') {
      //   res = await feedbackVoiceChatMessage(messageId, 0, data.reasons, data.text);
    }
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              {isSubmitted ? <SvgDisabledBad className="size-4" /> : <SvgBad className="size-4" />}
            </Button>
          </TooltipTrigger>
          <TooltipContent>
            <p>この回答は改善が必要です</p>
