...
export default async function feedbackSchedule(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource } = await scheduleContainer.item(id, user.id).read<ScheduleModel>();
    ...
    if (resource.feedbackType !== undefined) {
      throw new Error(`schedule with id ${id} is already fed back`);
    }

    await scheduleContainer.items.upsert({
      ...resource,
      feedbackType: ftype,
