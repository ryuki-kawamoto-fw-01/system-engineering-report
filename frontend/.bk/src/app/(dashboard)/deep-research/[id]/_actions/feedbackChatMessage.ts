...
import { deepMessageContainer } from '../../../../../../cosmos';

type Response = Result;

export default async function feedbackChatMessage(
  id: string,
  feedbackType: 0 | 1,
  ...
  try {
    ...
    const { resource: message } = await deepMessageContainer
      .item(id, user.id)
    ...
    await deepMessageContainer.items.upsert({
      ...message,
      feedbackType: ftype,
