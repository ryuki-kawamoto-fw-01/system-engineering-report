...
import { badFeedbackOptions, badFeedbacktext } from '../../../../../../config';
...
export default function FeedbackBadButton({
  ...
  const handleSubmit = useCallback(async (): Promise<void> => {
    ...
    const res = await feedbackChatMessage(messageId, 0, feedbackOptions, feedbackText);
    if (res.success) {
      onSubmit();
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <ThumbsDown
                size={14}
                className={cn(isSubmitted ? 'text-gray-500 fill-current' : '')}
              />
              <span className="sr-only">改善が必要な回答</span>
            </Button>
          </TooltipTrigger>
