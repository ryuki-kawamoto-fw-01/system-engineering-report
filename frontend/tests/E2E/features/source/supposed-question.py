"""
supposed-question.py - 想定質問画面のE2Eテスト

Azure Entra ID認証を含む、想定質問画面の完全なE2Eテストを実施します。
手動認証モードとヘッドレスモードの両方に対応しています。

【テスト観点】
① 想定質問画面が正しく表示されることを確認する
② カーソルをヘルプマークに合わせると機能説明が表示されることを確認する
③ 「ファイルを選択」から対象ファイルを選択できることを確認する
④ 対象ファイルをドラッグ＆ドロップで添付できることを確認する（複数拡張子テスト：docx, pdf, pptx, txt, xlsx）
⑤ 説明相手の特徴のスライダーが変更できることを確認する
⑥ 必要情報を入力し、想定質問が作成できることを確認する
⑦ 「想定質問を修正する」ボックスから、想定質問の内容を修正できることを確認する
...
■ スクリプトとして直接実行
  python frontend/tests/E2E/features/source/supposed-question.py
...
【エビデンス】
  ...
  ├── 01_supposed_question_page_display.png（①画面表示）
  ...
  ├── 04_file_drop_1_docx_attached.png / 04_file_drop_1_docx_removed.png（DOCX）
  ├── 04_file_drop_2_pdf_attached.png / 04_file_drop_2_pdf_removed.png（PDF）
  ├── 04_file_drop_3_pptx_attached.png / 04_file_drop_3_pptx_removed.png（PPTX）
  ├── 04_file_drop_4_txt_attached.png / 04_file_drop_4_txt_removed.png（TXT）
  ├── 04_file_drop_5_xlsx_attached.png（XLSX - 最終添付）
  ├── 04_file_drop_final.png（④全ファイルテスト完了）
  ├── 05_slider_change_1.png / 05_slider_change_2.png（⑤スライダー変更）
  ├── 06_form_before_submit.png / 06_result_after_creation.png（⑥想定質問作成）
  ├── 07_modify_result_1.png / 07_modify_result_2.png（⑦結果修正）
...
# テストファイルのディレクトリパス
TEST_FILES_DIR = Path(__file__).parent.parent / "input" / "03_supposed-question"
# STEP 7用の単一ファイル（最初のテスト用）
TEST_FILE_SINGLE = TEST_FILES_DIR / "説明資料.txt"


def test_supposed_question_functionality():
    """想定質問ページでファイル添付と想定質問作成のテストを実施"""
    ...
    with sync_playwright() as p:
        ...
        # /supposed-question ページに遷移
        print(f"\n[STEP 4] /supposed-questionページへ遷移")
        print(f"/supposed-questionページへ遷移します...")
        page.goto(f"{BASE_URL}/supposed-question")
        page.wait_for_load_state("networkidle")
        ...
        # 【テスト①】想定質問画面が正しく表示されることを確認
        print("\n[STEP 5] ①想定質問画面の表示確認")
        print("📸 エビデンス取得: 想定質問画面")
        save_screenshot(page, test_dir, "01_supposed_question_page_display.png", "想定質問画面の表示")
        
        try:
            # 【テスト②】ヘルプマークにカーソルを合わせると機能説明が表示される
            print("\n[STEP 6] ②ヘルプマーク機能説明の確認")
            try:
                # 想定質問タイトル内のヘルプコンポーネント
                help_button = page.locator('h3:has-text("想定質問")').locator('..').locator('button')
            ...
            try:
                ...
                test_files = sorted(TEST_FILES_DIR.glob("説明資料.*"))
                print(f"\n  ② {len(test_files)}個のファイルを順番にテスト")
                
                # テスト対象の拡張子の順序を定義（最後にxlsxが来るように）
                file_order = ['.docx', '.pdf', '.pptx', '.txt', '.xlsx']
                sorted_files = []
                ...
                if len(sorted_files) == 0:
                    ...
                    # 最終状態のスクリーンショット
                    save_screenshot(page, test_dir, "04_file_drop_final.png", "最終状態（説明資料.xlsxのみ添付）")
                    print(f"\n  ✓ 全{len(sorted_files)}ファイルのテスト完了")
                    print(f"  ✓ 最終状態: 説明資料.xlsxのみが添付された状態")
            ...
            try:
                ...
                # ファイルアップロード完了後に、説明の目的入力フィールドに入力
                description_field = page.locator('input[name="description"]')
                
                if description_field.count() > 0:
                    description_field.fill("新システム導入の提案")
                    page.wait_for_timeout(500)
                    print("✓ 説明の目的を入力しました")
                else:
                    warnings.append(("STEP 10", "説明の目的入力フィールドが見つかりませんでした"))
                    print("⚠️ 警告: 説明の目的入力フィールドが見つかりませんでした")
                
                # 考慮事項入力フィールド（considerationフィールド） - オプション
                consideration_field = page.locator('textarea[name="consideration"]')
                if consideration_field.count() > 0:
                    consideration_field.fill("初回提案の顧客を想定、質問は10個以内にまとめる")
                    page.wait_for_timeout(500)
                    print("✓ 考慮事項を入力しました")
                ...
                if submit_button.count() > 0:
                    ...
                    # 【エビデンス⑥-2】想定質問作成結果
                    print("\n[STEP 12] ⑥想定質問作成結果の確認")
                    print("📸 エビデンス取得: 想定質問作成結果")
                    save_screenshot(page, test_dir, "06_result_after_creation.png", "想定質問作成結果")
                    ...
                    if result_textarea.count() > 0:
                        ...
                        if len(result_text) > 50:
                            print(f"✓ 想定質問が作成されました（{len(result_text)}文字）")
                        else:
                            warnings.append(("STEP 12", f"想定質問の内容が短い可能性があります ({len(result_text)}文字)"))
                            print(f"⚠️ 警告: 想定質問の内容が短い可能性があります ({len(result_text)}文字)")
                    else:
                ...
                errors.append(("STEP 10", f"想定質問作成確認中にエラー: {e}"))
                print(f"❌ エラー: 想定質問作成確認中にエラー: {e}")
            
            # 【テスト⑦】「想定質問を修正する」ボックスから、想定質問の内容を修正できる
            print("\n[STEP 13] ⑦想定質問の修正機能確認")
            try:
                # 「想定質問を修正する」のTextareaを探す
                modify_textarea = page.locator('textarea[placeholder="作成結果を修正するための指示を入力してください"]')
                
                if modify_textarea.count() > 0:
                    save_screenshot(page, test_dir, "07_modify_result_1.png", "結果修正前")
                    
                    modify_textarea.fill("より具体的な技術的質問を追加する")
                    ...
                    if recreate_button.count() > 0:
                        ...
                        page.wait_for_timeout(3000)
                        save_screenshot(page, test_dir, "07_modify_result_2.png", "結果修正後（再作成完了）")
                    else:
                        warnings.append(("STEP 13", "再作成ボタンが見つかりませんでした"))
                        print("⚠️ 警告: 再作成ボタンが見つかりませんでした")
                else:
                    warnings.append(("STEP 13", "修正指示Textareaが見つかりませんでした"))
                    print("⚠️ 警告: 修正指示Textareaが見つかりませんでした")
            except Exception as e:
                errors.append(("STEP 13", f"想定質問修正確認中にエラー: {e}"))
                print(f"❌ エラー: 想定質問修正確認中にエラー: {e}")
                ...
                if result_area_buttons.count() >= 2:
                    ...
                    if feedback_dialog.count() > 0:
                        ...
                        if dialog_input.count() > 0:
                            dialog_input.fill("想定質問が的確で非常に役立ちました。")
                            page.wait_for_timeout(500)
                ...
                if result_buttons.count() >= 4:
                    ...
                    if cancel_button.count() > 0 and save_button.count() > 0:
                        ...
                        if editable_textarea:
                            ...
                            # テキストを変更
                            editable_textarea.fill("編集テスト：想定質問結果を更新しました")
                            page.wait_for_timeout(800)
                            print("✓ テキストを変更しました")
            ...
            try:
                ...
                if result_buttons.count() >= 3:
                    ...
                    try:
                        ...
                        # ファイル名が「想定質問_YYYYMMDD_HHMM.txt」形式か確認
                        if filename.startswith("想定質問_") and filename.endswith(".txt"):
            ...
            try:
                ...
                if clear_button.count() > 0:
                    ...
                    description_field = page.locator('input[name="description"]')
                    if description_field.count() > 0:
                        description_value = description_field.input_value()
                        if description_value == "":
                            print("✓ 入力情報が正しくクリアされました")
                        else:
                            warnings.append(("STEP 19", f"入力情報が残っている可能性があります: '{description_value[:50]}...'"))
                            print(f"⚠️ 警告: 入力情報が残っている可能性があります: '{description_value[:50]}...'")
                else:
            ...
            print(f"\n{'='*70}")
            print(f"✅ 全テスト完了: 想定質問画面の全機能を確認しました")
            print(f"{'='*70}")
...
if __name__ == "__main__":
    ...
    # メインテスト関数を実行
    print("=" * 70)
    print("Playwright E2Eテスト: 想定質問画面")
    print("=" * 70)
    
    try:
        print("\n[実行中] 想定質問機能テスト...")
        test_supposed_question_functionality()
        print("\n[✓ 成功] テストが正常に完了しました")
        sys.exit(0)
