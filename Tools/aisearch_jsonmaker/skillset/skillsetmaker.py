...
if __name__ == "__main__":
    print("=" * 60)
    print("Azure AI Search Skillset JSON Generator")
    print("=" * 60)
    ...
    template_file = os.path.join(script_dir, "data", "input", "skillset.json")
    csv_file = os.path.join(script_dir, "data", "input", "replacements.csv")
    output_dir = os.path.join(script_dir, "data", "output")
    archive_name = os.path.join(script_dir, "data", "output", f"generated_skillsets_{timestamp}.zip")
    
    # ステップ1: テンプレートファイルの確認
    print("【ステップ1】テンプレートファイルの確認")
    if not os.path.exists(template_file):
        print(f"✗ エラー: テンプレートファイルが見つかりません: {template_file}")
        print("data/input/skillset.json ファイルを作成してください。")
        exit(1)
    else:
    ...
    if not os.path.exists(csv_file):
        ...
        with open(csv_file, 'w', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['skillset_name', 'openai_resource_name', 'openai_deployment_id', 'index_name'])
            writer.writerow(['sample-skillset-1', 'my-openai-resource-1', 'text-embedding-3-large', 'sample-index-1'])
            writer.writerow(['sample-skillset-2', 'my-openai-resource-2', 'text-embedding-3-large', 'sample-index-2'])
    ...
    if replacements_preview:
        print(f"  → {len(replacements_preview)}件のスキルセット定義が見つかりました\n")
    else:
