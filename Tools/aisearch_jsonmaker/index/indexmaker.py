...
def generate_multiple_jsons(template_path: str, csv_path: str, output_dir: str) -> List[str]:
    ...
    for i, replacements in enumerate(replacements_list, 1):
        # 出力ファイル名を生成(index_nameがあればそれを使用)
        if 'index_name' in replacements:
            output_filename = f"{replacements['index_name']}.json"
        else:
            output_filename = f"index_{i}.json"
...
if __name__ == "__main__":
    print("=" * 60)
    print("Azure AI Search Index JSON Generator")
    print("=" * 60)
    ...
    template_file = os.path.join(script_dir, "data", "input", "index.json")
    csv_file = os.path.join(script_dir, "data", "input", "replacements.csv")
    output_dir = os.path.join(script_dir, "data", "output")
    archive_name = os.path.join(script_dir, "data", "output", f"generated_indexes_{timestamp}.zip")
    
    # ステップ1: テンプレートファイルの確認
    print("【ステップ1】テンプレートファイルの確認")
    if not os.path.exists(template_file):
        print(f"✗ エラー: テンプレートファイルが見つかりません: {template_file}")
        print("data/input/index.json ファイルを作成してください。")
        exit(1)
    else:
    ...
    if not os.path.exists(csv_file):
        ...
        with open(csv_file, 'w', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['index_name', 'vector_profile_name', 'semantic_name', 'algorithm_name', 'vectorizer_name', 'aoai_name', 'deployment_id'])
            writer.writerow(['sample-index-1', 'profile-1', 'semantic-1', 'algorithm-1', 'vectorizer-1', 'aoai-resource-1', 'deployment-1'])
            writer.writerow(['sample-index-2', 'profile-2', 'semantic-2', 'algorithm-2', 'vectorizer-2', 'aoai-resource-2', 'deployment-2'])
    ...
    if replacements_preview:
        print(f"  → {len(replacements_preview)}件のインデックス定義が見つかりました\n")
    else:
