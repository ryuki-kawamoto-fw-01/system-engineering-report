...
def generate_multiple_jsons(template_path: str, csv_path: str, output_dir: str) -> List[str]:
    ...
    # 現在時刻を取得（UTC、ISO 8601形式、ミリ秒2桁）
    from datetime import timezone
    current_time = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-4] + "Z"
    
    # 各設定でJSONを生成
    for i, replacements in enumerate(replacements_list, 1):
        # start_timeを自動的に追加
        replacements['start_time'] = current_time
        
        # 出力ファイル名を生成(indexer_nameがあればそれを使用)
        if 'indexer_name' in replacements:
            output_filename = f"{replacements['indexer_name']}.json"
        else:
            output_filename = f"indexer_{i}.json"
...
if __name__ == "__main__":
    print("=" * 60)
    print("Azure AI Search Indexer JSON Generator")
    print("=" * 60)
    ...
    template_file = os.path.join(script_dir, "data", "input", "indexer.json")
    csv_file = os.path.join(script_dir, "data", "input", "replacements.csv")
    output_dir = os.path.join(script_dir, "data", "output")
    archive_name = os.path.join(script_dir, "data", "output", f"generated_indexers_{timestamp}.zip")
    
    # ステップ1: テンプレートファイルの確認
    print("【ステップ1】テンプレートファイルの確認")
    if not os.path.exists(template_file):
        print(f"✗ エラー: テンプレートファイルが見つかりません: {template_file}")
        print("data/input/indexer.json ファイルを作成してください。")
        exit(1)
    else:
    ...
    if not os.path.exists(csv_file):
        ...
        # サンプルCSVを作成（indexer用、start_timeは自動設定）
        input_dir = os.path.join(script_dir, "data", "input")
        os.makedirs(input_dir, exist_ok=True)
        with open(csv_file, 'w', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['indexer_name', 'data_source_name', 'skillset_name', 'target_index_name'])
            writer.writerow(['indexer-documents-01', 'datasource-blob-documents-01', 'skillset-ocr-01', 'search-index-documents-01'])
            writer.writerow(['indexer-documents-02', 'datasource-blob-documents-02', 'skillset-ocr-02', 'search-index-documents-02'])
            writer.writerow(['indexer-documents-03', 'datasource-blob-documents-03', 'skillset-ocr-03', 'search-index-documents-03'])
        print(f"✓ サンプルCSVファイルを作成しました: {csv_file}")
        print(f"   ※ start_timeは自動的に現在時刻(UTC)が設定されます")
    else:
    ...
    if replacements_preview:
        print(f"  → {len(replacements_preview)}件のインデクサー定義が見つかりました\n")
    else:
