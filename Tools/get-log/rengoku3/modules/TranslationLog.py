...
def get_translation_df(uri, database_name, start_iso, end_iso):
    credential = DefaultAzureCredential()
    cosmos_client = CosmosClient(uri, credential=credential)
    database = cosmos_client.get_database_client(database_name)
    container = database.get_container_client("translation")
    ...
    for item in items:
        ...
        rec = {
            ...
            "テキスト": item.get("text", ""),
            "翻訳元言語": item.get("sourceLanguage", ""),
            "翻訳言語": item.get("targetLanguage", ""),
            "考慮事項": item.get("considerations", ""),
