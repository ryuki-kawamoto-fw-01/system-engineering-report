...
def get_create_idea_df(uri, database_name, start_iso, end_iso):
    credential = DefaultAzureCredential()
    cosmos_client = CosmosClient(uri, credential=credential)
    database = cosmos_client.get_database_client(database_name)
    container = database.get_container_client("create-idea")
    ...
    for item in items:
        ...
        rec = {
            ...
            "主題": item.get("ideationSubject", ""),
            "立場": item.get("ideationRole", ""),
            "アイデアの件数": item.get("ideationCount", ""),
            "考慮事項": item.get("ideationConsideration", ""),
