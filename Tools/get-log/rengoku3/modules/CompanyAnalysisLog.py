...
def get_company_analysis_df(uri, database_name, start_iso, end_iso):
    credential = DefaultAzureCredential()
    cosmos_client = CosmosClient(uri, credential=credential)
    database = cosmos_client.get_database_client(database_name)
    container = database.get_container_client("company-analysis")
    ...
    for item in items:
        ...
        rec = {
            ...
            "会社名": item.get("company_name", ""),
            "目的": item.get("analysis_purpose", ""),
            "事業名": item.get("business_name", ""),
            "考慮事項": item.get("analysis_considerations", ""),
