...
def get_text_correction_df(uri, database_name, start_iso, end_iso):
    credential = DefaultAzureCredential()
    cosmos_client = CosmosClient(uri, credential=credential)
    database = cosmos_client.get_database_client(database_name)
    container = database.get_container_client("text-correction")
    ...
    for item in items:
        ...
        rec = {
            ...
            "タイプ": item.get("type", ""),
            "ドキュメントタイプ": item.get("documentType", ""),
            "校正の観点": item.get("checkpoints", ""),
            "考慮事項": item.get("additionalConsiderations", ""),
            "出力項目": item.get("outputForm", ""),
            "文書校正": item.get("pointsOfCriticism", ""),
            "校正前の文章": item.get("originalText", ""),
