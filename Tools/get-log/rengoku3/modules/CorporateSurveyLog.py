...
def get_corporate_survey_df(uri, database_name, start_iso, end_iso):
    credential = DefaultAzureCredential()
    cosmos_client = CosmosClient(uri, credential=credential)
    database = cosmos_client.get_database_client(database_name)
    container = database.get_container_client("corporate-survey")
    ...
    for item in items:
        ...
        rec = {
            ...
            "会社名": item.get("surveyCompany", ""),
            "考慮事項": item.get("additionalConsideration", ""),
