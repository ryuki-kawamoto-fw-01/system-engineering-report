...
def get_summary_df(uri, database_name, start_iso, end_iso):
    credential = DefaultAzureCredential()
    cosmos_client = CosmosClient(uri, credential=credential)
    database = cosmos_client.get_database_client(database_name)
    container = database.get_container_client("summary")
    ...
    for item in items:
        ...
        rec = {
            ...
            "タイプ": item.get("type", ""),
            "要約する長さ": item.get("summaryLength", ""),
            "内容": item.get("content", ""),
            "考慮事項": item.get("consideration", ""),
