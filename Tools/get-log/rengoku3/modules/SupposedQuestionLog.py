...
def get_supposed_question_df(uri, database_name, start_iso, end_iso):
    credential = DefaultAzureCredential()
    cosmos_client = CosmosClient(uri, credential=credential)
    database = cosmos_client.get_database_client(database_name)
    container = database.get_container_client("supposed-question")
    ...
    for item in items:
        ...
        rec = {
            ...
            "説明資料": item.get("description", ""),
            "専門性": item.get("specialty", ""),
            "興味": item.get("interest", ""),
            "親密度": item.get("intimacy", ""),
            "入力項目": item.get("inputForm", ""),
